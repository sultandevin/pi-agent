 ...
 * Diff Extension
 *
 * Tracks files changed during agent runs (via git status + tool tracking)
 * and provides a /diff command to list and open changed files.
 *
 * Adapted from davis7dotsh/my-pi-setup.
 */
...
function parseGitStatus(output: string, cwd: string): Set<string> {
	...
	for (const line of output.split("\n")) {
		...
		// `git status --porcelain` format: two status columns, a space, then path.
		// Rename/copy entries look like `old -> new`; the destination is what we want.
...
export default function diffExtension(pi: ExtensionAPI): void {
	...
	pi.registerCommand(commandName, {
		...
		handler: async (args, ctx) => {
			...
			// Try zed first, fall back to $EDITOR or code
			const openers = [
				{ cmd: "zed", args: ["-e", file] },
				{ cmd: "code", args: ["-r", file] },
			];

			for (const opener of openers) {
				const result = await pi.exec(opener.cmd, opener.args, { cwd: ctx.cwd, timeout: 5000 });
				if (result.code === 0) {
					ctx.ui.notify(`Opened ${selected} in ${opener.cmd}`, "info");
					return;
				}
			}

			ctx.ui.notify(`Could not open ${selected} — no editor (zed/code) found`, "error");
		},
	});
}